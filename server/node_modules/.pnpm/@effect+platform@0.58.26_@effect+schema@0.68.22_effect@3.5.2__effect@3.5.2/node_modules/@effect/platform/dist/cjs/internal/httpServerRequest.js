...
const schemaBodyFormJson = (schema, options) => {
  ...
  return field => Effect.flatMap(serverRequestTag, request => {
    if (isMultipart(request)) {
      return Effect.flatMap(Effect.mapError(request.multipart, error => new Error.RequestError({
        request,
        reason: "Decode",
        error
...
class ServerRequestImpl extends Inspectable.Class {
  ...
  get stream() {
    return this.source.body ? Stream.fromReadableStream(() => this.source.body, _ => new Error.RequestError({
      request: this,
      reason: "Decode",
      error: _
  ...
  get multipartStream() {
    return Stream.pipeThroughChannel(Stream.mapError(this.stream, error => new Multipart.MultipartError({
